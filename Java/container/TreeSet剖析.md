...
1. TreeSet 是一个有序的集合，它的作用是提供有序的Set集合。它继承于AbstractSet抽象类，实现
2. NavigableSet<E>, Cloneable, java.io.Serializable接口。
...
7. TreeSet是基于TreeMap实现的。TreeSet中的元素支持2种排序方式：自然排序 或者 根据创建TreeSet 时提供
8. Comparator 进行排序。这取决于使用的构造方法。
9. TreeSet为基本操作（add、remove 和 contains）提供受保证的 log(n) 时间开销。
10. 另外，TreeSet是非同步的。 它的iterator 方法返回的迭代器是fail-fast的。
...
![image-20201231171320376](images/image-20201231171320376.png)

从图中可以看出：
(01) TreeSet继承于AbstractSet，并且实现了NavigableSet接口。
(02) TreeSet的本质是一个"有序的，并且没有重复元素"的集合，它是通过[TreeMap](http://www.cnblogs.com/skywang12345/p/3310928.html)实现的。TreeSet中含有一个"NavigableMap类型的成员变量"m，而m实际上是"TreeMap的实例"。
...
如果天空不死博客：https://www.cnblogs.com/skywang12345/p/3311268.html
